...
- Lambda はバージョン管理が可能
    - Lambda 関数を更新すると新しいバージョンとして管理される
    - 過去のバージョンも残るので、過去のバージョンを実行することも可能
...
- Lambda 関数のデプロイ方式として Zip 方式と コンテナイメージ方式がある

<br>

#### Zip 方式

- ★マネージドコンソール上で関数を編集してデプロイする場合も、裏でその関数ファイルが Zip にアーカイブされてからデプロイされる

    <img src="./img/Lambda-Deploy-Zip_1.png" />

<br>

#### コンテナイメージ方式

- コンテナイメージからデプロイする場合、ECR にイメージをプッシュしておく必要がある

- コンテナイメージからデプロイする場合は、当然だが Zip ファイルは生成されない

    <img src="./img/Lambda-Deploy-Container_1.png" />

<br>

#### デプロイ形式が違うと何が違うのか?

- アップロードできるファイル (プロジェクト)のサイズ上限が異なる
...
- 利用できない機能がある
    ...
    - Zip: [Layer](#レイヤー)、[コード署名](#コード署名)が利用可能

    - コンテナイメージ: [Layer](#レイヤー)、[コード署名](#コード署名)は利用不可
